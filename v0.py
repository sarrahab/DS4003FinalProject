...
# %%
# For salary-slider
# salary_marks = {int(salary): str(salary) for salary in sorted(df['Salary'].unique()[::5])}

salary_min = df['Salary'].min()
salary_max = df['Salary'].max()
# Find a representative middle value, for example, the median or mean, depending on your preference
...
# Assuming 'Age' is calculated and present in df
# age_marks = {int(age): str(age) for age in sorted(df['Age'].unique())}
...
app.layout = html.Div(children=[
    ...
    ], style={'width': '25%', 'display': 'inline-block', 'verticalAlign': 'top', 'padding': '10px', 'backgroundColor': '#f3f2f4'}), #light gray sidebar background
    
    html.Div([
        dcc.Graph(id='salary-engagement-scatter'),
        dcc.Graph(id='avg-satisfaction-bar'),  # Add this line for the bar chart
    ], style={'width': '70%', 'display': 'inline-block', 'backgroundColor': '#f3f2f4'}), #light gray background in main content area
])
